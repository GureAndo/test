...
//
foreach($tableauPrenoms as $key){
    echo $key . "<br>";
}

//function predefinie implode() 
//implode( )permet d'afficher les valeur d'un tableau sous forme de chaine de chara
// elle peut prendre un separateur ici(-) en argument pour separer - aerer les elements entre eux et dans le cas la ca sera le 1er argument (syntaxe obligatoire)
echo "<p>" . implode( ' - ',$listePays) . "</p>";

// les function prefdefinie sizeof() et count()
//chacune des ses function predefinie sert a compter le nombre d'element / valeur contenues dans un tableau
echo sizeof($tableauPrenoms) .'<br>';
echo count($tableauPrenoms) . '<br>'; 

// la boucle for() pour tableau 
//j'utilise count() pour dénombrer les éléùents de mon tableau et delimiter les conditions dans laquelle ma boucle for() doit s'executer.
...
echo "<br>";
// la meme avec la boucle foreach
...
// on parle de tableau associatif lorsqu'on decide de donner un 'nom' ou une lettre  a indice plutot que de laisser par defaut etre affiler a la valeur   
$listeCouleur = array('j' =>'jaune', 'r' => 'rouge', 'v'=>'vert');
echo "<pre>" ;print_r($listeCouleur); echo "</pre>";
// desormais pour crocherer je vais indiquer l'indice nomme pour recuperer ma valeur
echo $listeCouleur['v']. '<br>';
// je declare un nouveau tableau associatif(je nomme les indice)
$tableauFruit = array('fruit1' =>'orange', 'fruit2' => 'ringo' , 'fruit3' => 'fraise');
//cas particulier selonn que je crochete durant un concatenation (et donc avant de simple quotes)
echo 'fruit 1 : ' . $tableauFruit['fruit1'] . '<br>';
// ou selon que je crochete entre des bouble quotes
echo "fruit 2 :  $tableauFruit[fruit2] <br>";
// dans le 1er cas j'aurais besoin de simple quotes pour entourer ma valeur entre crochet. dans le second cas pas besoin de quotes. je corchete et j'apelle mon indice directement (si je mets des quote => erreur/warring=)

// tableau multidimensionnel
//un tableau multidimentionelle est un tableau qui contient d'autre tableau qui eu son a la fois des tableau et des incice du tableau mere 
$listeMarvel = array( 
 
    array('personnage' => 'spider-man', 'prenom' =>'peter', 'nom' =>'parker'),
    array('personnage' => 'iron man', 'prenom' =>'tony', 'nom' =>'stark'),
    array('personnage' => 'black widow', 'prenom' =>'natasha', 'nom' =>'romanoff'),
);

echo "<pre>"; var_dump($listeMarvel); echo"</pre>";

echo "<p>" . $listeMarvel[2]['prenom'].'</p><br';
...
//seconde syntaxd pour un tableau multidimentionell
//je vais aussi nommer les indices du tableau $tableauMarvel

$tableauMarvel = array(
    //au lien que php me donne un indice 0 par defaut pour le premier tableau je lui associe le nom de jessica jones
    'jessica jones' => array('prenom' => 'jessica', 'nom'=> 'jones'),
    //le second indice ne sera pas 1 par defaut mais le nom que je lui asscosie daredevil 
    'daredevil' => array('prenom' => 'matthew', 'nom'=> 'murdock'),
    //le troisieme indice ne sera pas 2 par defaut mais le nom que je lui asscosie hulk
    'hulk' => array('prenom' => 'bruce', 'nom'=> 'banner'),
);

// var_dump pour recuperer en affichage non conventionel les infi du tableau
echo '<pre>'; var_dump($tableauMarvel);echo'</pre>';
// pour afficher une donne precise je crochette toujours aux deux indice l'un deriere l'autre sauf que desormais le 1er est nommé aussi
echo "<p>" . $tableauMarvel['daredevil']['prenom'] . "</p><br>";

//por recuperer toute les donne de mon segon tableau je vais utiliser un double boucle foreach(boucle imbriquer)
...
echo "-------la meme boucle, sans afficher les indice du segond tableau-------- <br>";
//la meme double boucle avec une variation pour la recuperation des valeurs du segond tableau
foreach($tableauMarvel as $key => $value){
    echo "<ul>";
        if(is_array($value)){
            echo"<li>";
                echo"<h4>" . $key . "</h4>";
                //pour cette segonde for each, avec cette syntaxe, je vais recuperer directement les valeur (sans afficher leur indice)
                //si je ne script pas la fleche de corespondance =>, php traduit cela comme 'il ne veut que la valeur(meme si je l'ai appeler key2)
                // par contre si je script la 'as' (de 'alias') puis sa fleche de corespondance => alors il va m'afficher en premier lieu l'indice auquel il fera correspondre leur valeurs
                //comme toujour j'aurais pu choisir un autre nom de var
...
echo "<br>-----------------------les SuperGlobals---------------<br>";
//chapitre 12 les SuperGlobales
//elles ont la particulariter d'etre de type array, et cela aura son importance au nivceau de leur syntaxe, notament tout ce qui sera comprisentre les parenthese
//du fait que ce soit des tableau on va pouvoir les explorer avec print_r et var_dump
//elle sont disponible indifferament de l'espace global comme local sans syntaxe particuliere
// elle s'ecrive toute avec un _ entre $ et leur nom, hormais $GLOBALS (malgre qu'elle s'ecrivent en majuscule, ce sont des variable et non des constantes.)
//la superglobale $GLOBALS regroupe toute les autre(Superglobales),dont elle meme (d'ou le mot RECURSION dans sont array particulier)
echo "<pre>";print_r($GLOBALS);echo"</pre>";

echo '<br>-----------------------$_SERVICE---------------<br>';
//$_SERVER va me donner toute les information concernant le serveur 
echo "<pre>";print_r($_SERVER);echo"</pre>";
// dont une ,tres interessante que je pourais exploiter plus tard: DOCUMENT _ROOT.
//il contient la valeur du chemain vers le dossier htdocs.
//je pourais ainsi l'exploiter par exemple indiquer le chemain (dans une constante) vert un fichier img dans le quel j'uploadrais toute mes futures images que j'utiliseraios ensuite sur le site pour les afficher.
define('RACINE_SITE', $_SERVER['DOCUMENT_ROOT']. '/boutique/');

//chapitre 13 la methode GET

//la methode GET permet de faire transiter des info d'une page vers une a   utre par le biais de l'URL
// pour cela on va inclure dans l'atribut href de notre balise <a>
// je separe dans le href l'adresse du fichier d'acceuil avec la premiere info avec un ? 
//cette premiere information (enfait est un indice) je lui donne une valeur avec un = 

echo"<button><a href='page_get.php?produit=gateau&variete=chocolat&prix=12' target='_blank'>aller vers la page de recuperration</a></button>";  

//chapitre 14 la methode POST

echo'<br><br>--------------formulaire pour la méthode $_POST---------------<br><br>';
?>
...
<form class="my-5 col-4" method="post" action="">
  ...
  <select class="form-select mb-5" id="annee" name="annee">
    <?php
...
<button class='btn btn-primary'><a class='text-light' href='pageCookie.php' target='_blank'>Aller vers la page cookie</a></button>
